...
class WechatAuth {
     ...
     */
    public function getAccessToken($cache,$type = 'client', $code = null,$is_new = false){
        $param = array(
            'appid'  => $this->appId,
        ...
        //判断缓存有没有值
        if(!$is_new){
            $key =$this->appId."_access_token_".$type;
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                ...
                // set cache
                $cache->set($key, $token, $token['expires_in']);
     ...
     * 获取 Ticket
     */
    public function getTicket($cache,$type = 'jsapi')
    {
        $type = strtolower($type);
        if( !in_array($type, array('jsapi', 'wx_card'), true) ) {
            throw new \Exception(sprintf('Invalid Ticket Type: %s', $type));
        }
        ...
        $value = $this->_getTicket($cache,$type);
     ...
     * 获取 Ticket（从 API 获取）
     */
    protected function _getTicket($cache,$type)
    {   
        $access_token = $this->getAccessToken($cache);
        $access_token =$access_token['access_token'];
        $query = array(
                'access_token'  => $access_token,
                'type'          => $type
            );
        $request = self::http("{$this->apiURL}/ticket/getticket",  $query);
